...
class Minesweeper:
    ...
    def __init__(self, tk):
        ...
        self.labels["time"].grid(row = 0, column = 0, columnspan = RANGEOFY) # top full width
        self.labels["mines"].grid(row = RANGEOFX+1, column = 0, columnspan = int(RANGEOFY/2)) # bottom left of grid
        self.labels["flags"].grid(row = RANGEOFX+1, column = int(RANGEOFY/2)-1, columnspan = int(RANGEOFY/2)) # bottom right of grid
    ...
    def setup(self):
        ...
        self.tiles = dict({})
        self.mines = 0
        for x in range(0, RANGEOFX):
            for y in range(0, RANGEOFY):
                if y == 0:
                    self.tiles[x] = {} #only x values are added to the dictionary

                id = str(x) + "_" + str(y)
                isMine = False

                # image is titled changeable for debug reasons:
                gfx = self.images["plain"]

                # currently random amount of mines
                if random.uniform(0.0, 2.0) < 0.2:
                    isMine = True
                    self.mines += 1
    ...
    def refreshLabels(self):
        ...
        # loop is used again to find nearby mines and display number on tile
        for x in range(0, RANGEOFX):
            for y in range(0, RANGEOFY):
                mc = 0
    ...
    def gameOver(self, won):
        for x in range(0, RANGEOFX):
            for y in range(0, RANGEOFY):
                if self.tiles[x][y]["isMine"] == False and self.tiles[x][y]["state"] == STATE_IS_FLAGGED:
                    self.tiles[x][y]["button"].config(image = self.images["wrong"])
                if self.tiles[x][y]["isMine"] == True and self.tiles[x][y]["state"] != STATE_IS_FLAGGED:
    ...
    def onClick(self, tile):
        ...
        # change image
        if tile["mines"] == 0:
            tile["button"].config(image = self.images["clicked"]) #Changes to "clicked" image
            self.clearSurroundingTiles(tile["id"])
       
        else:
            tile["button"].config(image = self.images["numbers"][tile["mines"]-1])
        # if not already set as clicked, change state and count
        if tile["state"] != STATE_IS_CLICKED:
            tile["state"] = STATE_IS_CLICKED
            self.clickedCount += 1
       
        if self.clickedCount == (RANGEOFX * RANGEOFY) - self.mines:
            self.gameOver(True)
    ...
    def onRightClick(self, tile):
        ...
        # if not clicked
        if tile["state"] == STATE_IS_DEFAULT:
            tile["button"].config(image = self.images["flag"])
            tile["state"] = STATE_IS_FLAGGED
